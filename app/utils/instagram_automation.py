...
class InstagramAutomation:
    ...
    def post_reel(self, device_id, video_path, caption, music_query=None):
        ...
        try:
            ...
            try:
                ...
                # Set proper file permissions (ignore errors — Android FUSE mounts
                # return exit code 1 for chmod even on success)
                subprocess.run([
                    'adb', '-s', device_id, 'shell',
    ...
    def _download_media_from_url(self, url, media_type='photo'):
        ...
        try:
            ...
            # Use UPLOAD_FOLDER from config if available, fall back to app root/uploads
            try:
