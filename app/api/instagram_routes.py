...
def add_instagram_account():
    ...
    try:
        ...
        if not device:
            return jsonify({'error': f'Device with ID {data["device_id"]} not found'}), 404
...
def add_instagram_accounts_bulk():
    """Add multiple Instagram accounts to the database"""
    try:
        data = request.get_json()
        ...
        if not isinstance(accounts, list):
            return jsonify({'error': 'Accounts must be provided as a list'}), 400
        ...
        for account_data in accounts:
            ...
            if not all(field in account_data for field in ['username', 'password', 'device_id']):
                results['failed'].append({
                    'account': account_data.get('username', 'Unknown'),
                    'error': 'Missing required fields (username, password, device_id)'
                })
                continue

            # Check if account exists
            existing_account = InstagramAccount.query.filter_by(username=account_data['username']).first()
            if existing_account:
                results['failed'].append({
            ...
            # Check if device exists
            device = Device.query.get(account_data['device_id'])
            if not device:
                results['failed'].append({
                    'account': account_data['username'],
                    'error': f'Device with ID {account_data["device_id"]} not found'
                })
                continue
        ...
        return jsonify({
            'success': True,
            'message': f"Added {len(results['success'])} accounts, {len(results['failed'])} failed",
            'results': results
        })
